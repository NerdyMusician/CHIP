...
namespace CyberpunkGameplayAssistant.Models
{
    public partial class Combatant : BaseModel
    {
        ...
        {
            if (string.IsNullOrEmpty(ammoVar)) { ammoVar = AppData.AmmoVarBasic; }
        ...
        public void AddWeaponOptionsToWeapons()
        {
            ...
            if (WeaponOptions.Count < WeaponOptionsAllowed) { RaiseError(AppData.ErrorNotEnoughWeaponOptions); }
            List<WeaponOption> options = new(WeaponOptions);
            for (int i = 0; i < WeaponOptionsAllowed; i++)
            {
                WeaponOption weaponOption = ManualWeaponOptionSelection ? SelectManualWeaponOption(options) : options[AppData.RNG.Next(0, options.Count)];
